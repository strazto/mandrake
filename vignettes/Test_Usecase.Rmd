---
title: "Test_Usecase"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Test_Usecase}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r setup}
library(mandrake)
library(drake)
library(knitr)
library(magrittr)

source("mtcars/R/functions.R")
```


```{r plan}
# This is where you set up your workflow plan,
# a data frame with the steps of your analysis.

my_plan <- drake::drake_plan(
  report = knit(knitr_in("mtcars/report.Rmd"), file_out("mtcars/report.md"), quiet = TRUE),
  small = simulate(48),
  large = simulate(64),
  regression1 = target(
    reg1(data),
    transform = map(data = c(small, large), .tag_out = reg, .tag_in = cluster_id)
  ),
  regression2 = target(
    reg2(data),
    transform = map(data, .tag_out = reg, .tag_in = cluster_id)
  ),
  summ = target(
    suppressWarnings(summary(reg$residuals)),
    transform = map(reg, .tag_in = cluster_id)
  ),
  coef = target(
    suppressWarnings(summary(reg))$coefficients,
    transform = map(reg, .tag_in = cluster_id)
  ),
  
  mtcars = target(
    mtcars
  ),
  trace = TRUE
)

my_plan %>% 
  dplyr::mutate(dplyr::across("command", rlang::as_label)) %>%
  knitr::kable("html")
```

```{r}
cache <- drake::get_cache()
my_config <- drake_config(my_plan)
drake::make(config = my_config)

```

```{r}
lookup_cache <- mandrake::load_package_colspec("mandrake")

plan_extracted <- my_plan %>%
  extract_column_names(cache, group = "cluster_id") 
```
```{r}
plan_extracted %<>%
  dplyr::mutate(
    target_column_list = link_col2doc(target_column_list, lookup_cache = lookup_cache))
```

```{r}
my_config <- drake_config(plan_extracted)
```

```{r, include=FALSE}
alert_behaviour <- function(){
  js <- "
  function(props) {
    node = this.body.data.nodes.get(props.nodes[0]);
    cr = '\\r\\n';
    alert('selected ' +  node.label + ':' + cr +
            '=======' + cr +
            'COLNAMES:' + cr +
           node.on_select_col + cr + 
           '=============== '
           );
  }"
}
```

```{r}
embed_behaviour <- function() {
  glue::glue(
    "function(props) {",
      "container = this.body.container;",
      "legend = $(container).prev()[0];",
      "node = this.body.data.nodes.get(props.nodes[0]);",
      "$(legend).css({'font-size': '10px', 'width' : '30%'});",
      "$(container).css('width', '70%');",
      "legend.innerHTML = node.on_select_col;",
    "}",
    .sep = "\n",
    .open = "<{",
    .close = "}>"
  ) %>% as.character()
}

```


```{r}
graph_info <- drake_graph_info(
  my_config, 
  group = "cluster_id", 
  clusters = c("summ", "coef"), 
  build_times = "none",
  on_select_col = "target_column_list")

graph <- render_drake_graph(
  graph_info, 
  on_select = embed_behaviour(),
  ncol_legend = 2
  ) %>%
  visNetwork::visHierarchicalLayout(
    direction = "LR", levelSeparation = 250
    ) %>%
  visNetwork::visEdges(
    smooth = list(type = "cubicBezier", forceDirection = "horizontal")
    )
```

# Graph
```{r}
graph
```
